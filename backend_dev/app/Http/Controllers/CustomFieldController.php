...
class CustomFieldController extends Controller
 {
    ...
    {
        // Check if field_options are needed (for form rendering)
        $includeOptions = $request->get('include_options', false);
        
        if ($includeOptions) {
            // Load all fields including field_options for form
            $customFields = CustomField::where('is_active', true)
                ->orderBy('sort_order')
                ->get();
        } else {
            // Exclude field_options JSON to reduce payload size for list view
            $customFields = CustomField::select('id', 'field_name', 'field_type', 'is_active', 'sort_order')
                ->orderBy('sort_order')
                ->get();
        }
